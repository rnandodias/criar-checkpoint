...
- **Formato da prova prática — `--formato projeto|cases` (etapa 4):** `projeto` (default) = projeto prático de implementação (fluxo testado, `system_prompt_aula3_txt`). `cases` = prova de **ANÁLISE DE CASES** para carreiras conceituais/analíticas, via `system_prompt_aula3_cases_txt` + `user_prompt_aula3_cases_txt` (funções **isoladas** — não tocam no prompt de projeto). O formato cases gera **um case único evolutivo** (sem código/datasets), entregáveis documentais como **sugestões** (não checklist), **análise aberta** (sem gabarito — traz "O que caracteriza uma boa análise" em cada etapa) e **IA como copiloto crítico** por etapa. Reusa o mesmo esqueleto de seções, então a etapa 5 publica sem alterar o parser. Combine com `--perfil conceitual --modo_dados sem`. Estreou em Arquitetura de Soluções com IA n1 (2026-07). No QA cego dessa prova, use **rubrica de ANÁLISE** (coerência do case entre etapas, qualidade das tarefas, IA-copiloto calibrada, sem gabarito vazado), NÃO a de executabilidade de dataset — e a **cobertura dos cursos é avaliada por quem tem os resumos** (o revisor cego não os vê).
  ...
  - **A prova prática NÃO tem correção nem gabarito para a pessoa aluna — desenhe para isso (Gov Dados n3, 2026-09).** Defeito plantado vira gabarito oculto que ela nunca vê. Por isso, no reforço (modelo: `output/governanca_de_dados_nivel_3/_reforco_artefatos_gd3.txt`): **sem pegadinha** ("trecho correto que parece suspeito" só ensina com feedback — tire); cada defeito rastreável a um princípio ensinado; "O que caracteriza uma boa análise" = **guia de autoavaliação** com critérios genéricos (inclusive de priorização); missões de julgamento/priorização, não caça ao erro; **IA como segunda leitura** depois do diagnóstico próprio; instrução para marcar "[a levantar com …]" o que o case não traz, em vez de inventar. Valide com **teste de resolvedor** (subagente cego resolve a prova e registra atritos) — em Gov Dados n3 deu "resolvível, 13,5–19 h".
  - **Ferramentas em cases:** a pessoa não precisa de ferramenta para analisar e escrever. Passe `--ferramentas_arquivo` com só "assistente de IA generativa" + "editor de documentos, planilhas ou diagramas de sua preferência"; plataformas estudadas (Purview, Collibra, Glue…) entram no reforço como **contexto do case** — exceção explícita à regra dura de autoridade da lista, que sem isso as proibiria de aparecer.
...
- **NADA de conteúdo é cortado (regra do usuário, 2026-09-24).** Até então a etapa 4 cortava o JSON dos resumos em 300k chars **sem aviso** — limite arbitrário (o Opus 4-6 aceita 1M tokens de entrada / 128k de saída) — e só os primeiros cursos da trilha chegavam ao gerador da prática: afetou 16 de 20 checkpoints (os "5 cursos-âncora" de Gov Dados n2 eram simplesmente os 5 que sobreviviam ao corte). Hoje: (a) insumo vai **completo**; o teto `INSUMO_MAX_CHARS` (2,4M chars) só existe para **abortar com mensagem** se não couber — quem decide o recorte é o usuário; vale para prática, revisor 4.5, teórica (por curso) e turbinador 4.7 (que cortava o enunciado em 12–14k chars); (b) resposta com `stop_reason == "max_tokens"` é **cortada** e nunca é aproveitada — no batch (etapas 2/3) é reenviada com o dobro do teto até 32.768 e depois aborta; no síncrono aborta; na prática o batch usa `max_tokens` 32.000. Ao mexer em qualquer script, procure `[:N]` e `max_tokens` sem checagem e **avise o usuário** de qualquer ponto onde conteúdo possa se perder. Amostra deliberada de dados (`df.head(N)`, datasets inline de 30–120 linhas) não é corte de conteúdo de curso.
- **Resumos são por curso e independentes da carreira** (o prompt da etapa 2 não usa carreira nem nível). Curso repetido entre carreiras (ex.: 5708 e 5710 em IA Automação n3 e Gov Dados n3) pode ter o resumo reaproveitado: rodar a etapa 2 numa trilha temporária `_parcial_*.json` só com os cursos novos e juntar na ordem da trilha — confira antes que as transcrições são idênticas.
- **`--ajustar_alternativas` (teórica) NÃO usar** até o prompt ser refeito: dispara só com diferença ≥20% de tamanho (4–6 de 20 questões, enquanto a correta é a mais longa em 13–19/20) e o few-shot "GOOD" do prompt alonga distratores com assunto de OUTRA questão, o que facilita descartá-los. O viés "correta mais longa" se trata no QA, com reescrita cirúrgica.
...
- **`criar_secoes` deixa a seção "Prova teórica" INATIVA.** Efeito colateral de `marcar_prova_teorica`, que salva o formulário da seção ao marcar "É prova?". Aconteceu em quase todos os checkpoints observados (Eng ML n1 e os três da carreira IA aplicada a Operações e Infraestrutura) — mas em Gov Dados n3 (curso 5258, 2026-09) a seção ficou **ATIVA**. Confira o status real em vez de presumir; com o fluxo limpo (teóricas criadas ativas + desativação seletiva) a seção ativa já mostra só as 10 escolhidas. **Não reative antes de ativar as questões:** com as 20 teóricas em INACTIVE, uma seção ativa mostra ao aluno uma prova vazia. Ordem correta: ativar as 10 questões escolhidas → depois ativar a seção.
